 ...
 * ocrPipeline.js
 *
 * The central OCR ingestion pipeline for VaultHive.
 *
 * Responsibilities
 * ─────────────────
 * 1. Preparation  — client-side image resize / base64 before network call
 * 2. Transport    — invoke vaulthive-ocr Edge Function (image OR pdf)
 * 3. Normalisation — strip RTL marks, unify Arabic digits, collapse whitespace
 * 4. Parsing      — extract vendor / date / amount / serial / warranty dates
 * 5. Merchant normalisation — alias lookup → canonical merchant row
 * 6. Category suggestion — merchant history → keyword rules → user category list
 * 7. Confidence scoring — per-field weak/strong flags for UI affordance
 * 8. State machine — idle → running → success | partial | failed
 *
 * Contract
 * ─────────
 * runOcrPipeline(input, options) → OcrPipelineResult
 *
 * OcrPipelineResult
 * {
 *   status:     'success' | 'partial' | 'failed' | 'not_configured'
 *   raw_text:   string          — verbatim normalised OCR output (always kept)
 *   parsed:     ParsedReceipt | ParsedWarranty | null
 *   suggestions: FieldSuggestions — per-field with value + confidence
 *   merchant:   { merchant_id, name, verified } | null
 *   error:      string | null   — human-readable, safe to show in UI
 *   provider:   string          — 'server-ocr' | 'client-fallback' | 'none'
 *   retryable:  boolean
 * }
 *
 * FieldSuggestions (receipt)
 * {
 *   vendor_name:      { value, confidence: 'high'|'medium'|'low' }
 *   purchase_date:    { value, confidence }
 *   total_amount:     { value, confidence }
 *   currency_code:    { value, confidence }
 *   category_id:      { value, confidence, reason }
 * }
 *
 * Backend assumptions
 * ────────────────────
 * • The vaulthive-ocr Edge Function accepts { imageDataUrl?, fileUrl?, mimeType?,
 *   filename?, languageHint? } and returns { text, provider, error? }.
 * • The function handles both Vision API (images) and document AI (PDFs) server-side.
 * • No structured extraction is done server-side — all parsing happens here.
 * • Future: an `extract` mode flag can be added to body so the server returns
 *   JSON fields directly; the pipeline detects this via data.structured?.fields
 *   and short-circuits client-side parsing (backend_orchestration_ready flag).
 */
...
// Category keyword map — mirrors the default category_defaults seed
