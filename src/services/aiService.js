...
export function isConfigured() {
  return (
    (!!endpoint && /^https?:\/\//i.test(String(endpoint).trim())) ||
    !!supabase
  );
}
...
async function invokeAiEndpoint(requestBody) {
  ...
  console.log("[AI] Invoking direct endpoint:", url);
...
async function invokeSupabaseFunction(requestBody) {
  if (!supabase) {
    throw new Error("Supabase client is not configured");
  }

  console.log("[AI] Invoking Supabase function vaulthive-ai via supabase-js");
  console.log("[AI] Request body size:", JSON.stringify(requestBody).length, "bytes");

  const { data, error } = await supabase.functions.invoke("vaulthive-ai", {
    body: JSON.stringify(requestBody),
    headers: {
      "Content-Type": "application/json",
      "Accept": "application/json",
      "Authorization": SUPABASE_ANON_KEY ? `Bearer ${SUPABASE_ANON_KEY}` : undefined,
      "apikey": SUPABASE_ANON_KEY || undefined,
    },
  });

  if (error) {
    console.error("[AI] Supabase function error:", error.message || error);
    throw new Error(error.message || "Supabase function error");
  }

  if (!data) {
    console.error("[AI] Supabase function returned no data");
...
async function chat({ uid, message, history = [], context = null, attachment = null }) {
  if (context) {
    await attachStoreCandidates(context, uid, message).catch((err) => {
      console.warn("[AI] Failed to attach store candidates:", err);
    });
  }
  ...
  if (!isConfigured()) {
    console.warn("[AI] Endpoint not configured, using local fallback");
    return buildLocalFallback({ message, attachment: preparedAttachment, context });
  }

  try {
    const requestBody = buildRequestBody({ uid, message, history, context, attachment: preparedAttachment });

    if (endpoint) {
      try {
        const data = await invokeAiEndpoint(requestBody);
        const normalizedResponse = normalizeRemoteResponse(data, context);
        console.log("[AI] Response normalized successfully");
        return normalizedResponse;
      } catch (endpointErr) {
        console.error("[AI] Direct endpoint failed:", endpointErr?.message || endpointErr);
        if (supabase) {
          console.log("[AI] Falling back to Supabase function invocation");
          const data = await invokeSupabaseFunction(requestBody);
          const normalizedResponse = normalizeRemoteResponse(data, context);
          console.log("[AI] Response normalized successfully via Supabase function");
          return normalizedResponse;
        }
        throw endpointErr;
      }
    ...
    if (!supabase) {
      console.warn("[AI] Supabase client is not configured, falling back to local response.");
      return buildLocalFallback({ message, attachment: preparedAttachment, context });
    }
    ...
    if (errorMsg.includes("AbortError")) {
      console.error("[AI] Request timeout (30s): ", endpoint);
    } else if (errorMsg.includes("Network")) {
      console.error("[AI] Network error - check your connection and OPENAI_API_KEY on server");
    } else {
      console.error("[AI] Error calling remote endpoint:", errorMsg, err);
    }
    console.log("[AI] Falling back to local response");
