...
export function useAIConversation({ linkedEntityType = null, linkedEntityId = null, screenContext = "ai_assistant" } = {}) {
  ...
  const sendMessage = useCallback(
    async ({ text, attachment = null }) => {
      ...
      if (!ctx) {
        try { 
          ctx = await buildAIContext(uid, { forceRefresh: false }); 
        } catch (err) {
          console.warn("[AI] Failed to build context:", err);
          ctx = null;
        }
      ...
      let aiResponse = null;
      try {
        console.log("[AI] Calling chat service...");
        ...
        console.log("[AI] Chat service response received:", aiResponse);
      } catch (err) {
        const errorMsg = String(err?.message || err || "Unknown error");
        console.error("[AI] Chat service error:", errorMsg);
        aiResponse = {
          reply: `Sorry, I couldn't process that request: ${errorMsg}. Please try again.`,
          sections: [],
          cards: [],
          suggestions: [],
          action_proposals: [],
      ...
      try {
        ...
        console.error("[AI] Failed to persist assistant message:", err);
      }
